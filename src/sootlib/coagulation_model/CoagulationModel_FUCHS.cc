...
double CoagulationModel_FUCHS::getCoagulationRate(const InputState& state,
    ...
    const double Cc1 =
        1 + Kn1 * (1.257 + 0.4 * exp(-1.1 / Kn1));       // Seinfeld p. 372 eq. 9.34. This is for air at 298 K, 1 atm
    const double Cc2 = 1 + Kn2 * (1.257 + 0.4 * exp(
        -1.1 / Kn2));       // for D<<mfp_g, Cc = 1 + 1.657*Kn; Seinfeld p. 380: 10% error at Kn=1, 0% at Kn=0.01, 100
    ...
    const double
        g1 = sqrt(2.0) / 3.0 / Dp1 / l1 * (pow(Dp1 + l1, 3.0) - pow(Dp1 * Dp1 + l1 * l1, 3.0 / 2.0)) - sqrt(2.0) * Dp1;
    const double
        g2 = sqrt(2.0) / 3.0 / Dp2 / l1 * (pow(Dp2 + l2, 3.0) - pow(Dp2 * Dp2 + l2 * l2, 3.0 / 2.0)) - sqrt(2.0) * Dp2;

    return 2.0 * M_PI * (D1 + D2) * (Dp1 + Dp2) / ((Dp1 + Dp2) / (Dp1 + Dp2 + 2.0 * sqrt(g1 * g1 + g2 * g2))
        + 8.0 / eps_c * (D1 + D2) / sqrt(c1 * c1 + c2 * c2) / (Dp1 + Dp2));
