...
double CoagulationModel_FRENK::getCoagulationRate(const InputState& state,
    ...
    const double beta_12_FM = eps_c * sqrt(M_PI * kb * state.getT() * 0.5 / m12) * pow(Dp1 + Dp2, 2.0);
    ...
    const double
        Cc1 = 1 + Kn1 * (1.257 + 0.4 * exp(-1.1 / Kn1));   // Seinfeld p. 372 eq. 9.34. This is for air at 298 K, 1 atm
    const double Cc2 = 1 + Kn2 * (1.257 + 0.4
        * exp(-1.1 / Kn2));   // for D<<mfp_g, Cc = 1 + 1.657*Kn; Seinfeld p. 380: 10% error at Kn=1, 0% at Kn=0.01, 100

    double beta_12_C = 2.0 * kb * state.getT() / (3.0 * state.getMuGas()) * (Cc1 / Dp1 + Cc2 / Dp1) * (Dp1 + Dp2);
