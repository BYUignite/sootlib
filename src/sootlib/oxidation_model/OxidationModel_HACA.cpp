...
double OxidationModel_HACA::getOxidationRate(const MomentState& state, MassRateRatios& ratio) const {
    ...
    const double fR6 = 1290.0 * 0.13 * state.getP() * (cOH / state.getRhoGas() * MW_OH)
        / sqrt(state.getT());    // gamma = 0.13 from Neoh et al.
    ...
    const double Roxi =
        -fR1 * c_soot_H + rR1 * c_soot_rad - fR2 * c_soot_H + rR2 * c_soot_rad + fR3 * c_soot_rad + fR4 * c_soot_rad
            - fR6 * c_soot_H;    // #-available-sites/m2-mix*s
