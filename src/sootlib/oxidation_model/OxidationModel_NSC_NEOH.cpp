...
double OxidationModel_NSC_NEOH::getOxidationRate(const MomentState& state, MassRateRatios& ratio) const {
    ...
    const double kA = 20.0 * exp(-15098.0 / state.getT());              // rate constants
    const double kB = 4.46E-3 * exp(-7650.0 / state.getT());
    const double kT = 1.51E5 * exp(-48817.0 / state.getT());
    const double kz = 21.3 * exp(2063.0 / state.getT());
