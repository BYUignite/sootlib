...
double OxidationModel_LEE_NEOH::getOxidationRate(const MomentState& state, MassRateRatio& ratio) const {
    ...
    const double pOH_atm = state.getGasSpeciesP(GasSpecies::OH)
        / 101325.0;                                     // partial pressure of OH (atm)

    const double
        rSootO2 = 1.085E4 * pO2_atm / sqrt(state.getT()) * exp(-1.977824E4 / state.getT()) / 1000.0;   // kg/m^2*s
    const double
