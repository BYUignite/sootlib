...
double GrowthModel_HACA::getGrowthRate(const State& state, MassRateRatios& ratio) const {
    ...
    const double fR1 = 4.2E13 * exp(-13.0 / RT) * state.getGasSpeciesC(GasSpecies::H) / 1000;
    const double rR1 = 3.9E12 * exp(-11.0 / RT) * state.getGasSpeciesC(GasSpecies::H2) / 1000;
    ...
    const double fR6 = 1290.0 * 0.13 * state.getP() * (state.getGasSpeciesC(GasSpecies::OH) / state.getRhoGas() * MW_OH) / sqrt(state.getT());    // gamma = 0.13 from Neoh et al.
    ...
    double alpha = 1;                                            // alpha = fraction of available surface sites
    if (M0 > 0)
