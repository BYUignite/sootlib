...
SourceTerms SootModel_SECT::getSourceTerms(InputState& state) const {
    ...
    vector<double> wts = state.getMomentsConst();
    vector<double> absc(state.getNumMoments(), 0);
    ...
    vector<double> Coag(state.getNumMoments(), 0);
    double leaving;
    ...
    for (size_t i = 0; i < Coag.size(); i++) {
        for (size_t j = 0; j < Coag.size(); j++) {
            ...
            Coag.at(i) -= leaving;
            Coag.at(j) -= leaving;
            // FIXME the old code references "sections" which is a vector that doesn't exist. I think this is either bins or moments
            divided = getDivision((state.getMoment(i) + state.getMoment(j)), leaving, absc);
    ...
    vector<double> N0(state.getNumMoments(), 0);
    N0.at(0) = Jnuc;
    const double N_tot = Jnuc * state.getCMin() * MW_C / Na;

    vector<double> Cnd0(state.getNumMoments(), 0);
    double Cnd_tot = 0;
    ...
    vector<double> Am2m3(state.getNumMoments(), 0);
    for (size_t i = 0; i < state.getNumMoments(); i++) {
        if (wts.at(i) > 0)
            Am2m3.at(i) = M_PI * pow(abs(6.0 / (M_PI * state.getRhoSoot()) * state.getMoment(i)), 2.0 / 3) * abs(wts.at(i));
    }
    vector<double> G0(state.getNumMoments(), 0);
    double G_tot = 0;
    double Ngrw;
    for (size_t i = 0; i < state.getNumMoments(); i++) {
        if (i == 0)
            Ngrw = -Kgrw.at(i) * Am2m3.at(i) * wts.at(i) / (absc.at(i + 1) - absc.at(i));
        else if (i == (state.getNumMoments() - 1))
    ...
    vector<double> X0(state.getNumMoments(), 0);
    double X_tot = 0;
    double Noxi;
    for (size_t i = 0; i < state.getNumMoments(); i++) {
        if (i == 0)
