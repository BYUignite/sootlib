...
double oxidationModel_LEE_NEOH::getOxidationSootRate(const state &state) const {
    ...
    const double pO2_atm = state.getGasSpeciesP(gasSp::O2) / 101325.0;     // partial pressure of O2 (atm)
    const double pOH_atm = state.getGasSpeciesP(gasSp::OH) / 101325.0;     // partial pressure of OH (atm)
