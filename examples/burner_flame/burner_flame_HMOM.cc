...
int main(int argc, char** argv) {
    ...
    size_t nsoot = 8; // 7 moments + 1 weight
    ...
    ofstream ofile("burner_HMOM.out");

    //---------- integrate: second order midpoint method

    cout << endl << "********* Writing output to burner_HMOM.out" << endl;

    ofile << setprecision(2) << fixed;
    ofile << setprecision(4) << scientific;
    ofile << "# z(m), M0(#/m3), M1(kg/m3), M2(kg^2/m3), N0(#/m3)";
    ...
    for(int istep = 0; istep < nsteps; istep++, z+=dz) {
        ...
        for(int i=0; i<nsoot; i++)
            M[i] = Mhat[i]*LI_rho(z);
        S.setState(LI_T(z), P, LI_rho(z), LI_mu(z), yGas, yPAH, M, nsoot, vector<double>(), 0, 1667.0);
        SM.setSourceTerms(S);
        ...
        S.setState(LI_T(zh), P, LI_rho(zh), LI_mu(zh), yGas, yPAH, M, nsoot, vector<double>(), 0, 1667.0);
        SM.setSourceTerms(S);
